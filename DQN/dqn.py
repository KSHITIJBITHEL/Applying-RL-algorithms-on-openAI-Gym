...
def mini_batch_train(env, agent, max_episodes, max_steps, batch_size):
    ...
    for episode in range(max_episodes):
        state = env.reset()
        episode_reward = 0
        if eps > 0.1 and last_update- episode >30:
                eps *= 0.97
                last_update = episode
...
episode_rewards,avg_rewards = mini_batch_train(env, agent, MAX_EPISODES, MAX_STEPS, BATCH_SIZE)
plt.plot(np.arange(np.size(episode_rewards)) , episode_rewards)
plt.show()
plt.plot(np.arange(np.size(avg_rewards)) , avg_rewards)
plt.show()